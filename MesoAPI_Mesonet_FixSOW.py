...
# Updated (09/30/2015):
#----------------------------------------------------------------------------------------
def StateOfWeather(X,row,SRtable):
    #print X['Ob Date']
    #print row
...
##If (OBS_SOLAR < 25) SOW = 3 Catch observations during the nighttime.
    ...
    if CurrentHourPrecip <= 0.1: ##the current hour precipitaiton is 0 and use solar radiation for get the SOW
        ...
        PCNT_SOLAR = X['SolarRad']/MaxHourSR
        ...
        if X['SolarRad'] < 25:
            ...
            ##3---Overcast:
                ...
                ##If (PCNT_SOLAR < Pcnt_Broken) SOW = 3 (Overcast, > 9/10 cloud cover)
                    ...
                    X['State of Weather'] = 3
        print X['State of Weather'],X['Ob Time'],PCNT_SOLAR,'MaxHourSR:',MaxHourSR,'SolarRad:',X['SolarRad'],'Moisture:',X['Moisture']

    ##If precipitation for the current hour bigge than 0.1, determine the SOW based on rain amount
    else:
