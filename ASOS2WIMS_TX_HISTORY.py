...
#------------------------------------------------------------------------------------------------------------------
##def windspeed(gust):
##    #return gust * math.log(6/0.0984)/math.log(10/0.0984)
...
def windspeed(wind_speed_kt):
    return wind_speed_kt * 0.9 * math.log(6/0.0984)/math.log(10/0.0984)
...
#def IntepreteHistASOS(STATION,ID):
    ...
    with open(histASOS) as filt_csv:
        ...
        asos = asos.drop_duplicates(['valid'], take_last=True)
        ...
        fileWF9 = WorkSpace + '\\tx-asos-' + STATION + '.fw9'
...
WorkSpace = os.path.join(WorkSpaceRoot, "HIST")
...
starts = datetime(2015, 10, 26)
endts = datetime(2015, 10, 29)
...
for STATION,ID in Stations.items():
    ...
    outfn = WorkSpace + '\\%s_%s_%s.txt' % (station, starts.strftime("%Y%m%d%H%M"),
                              endts.strftime("%Y%m%d%H%M"))

    try:
        #Download the historical asos for each station
        urllib.urlretrieve(uri,outfn)
        #histASOS = WorkSpace + "\\" + outfn
        #Parse the information into FW9 format
